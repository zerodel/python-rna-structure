...
DataSetFilter filteredData = CreateFilter (nucleotideSequences,3,"","", GeneticCodeExclusions);
...
#include "GY94.mdl";
Tree tree4gy = TREE_NEWICK_STRING;

LikelihoodFunction lf0 = (filteredData, tree4gy);

fprintf (stdout, "\n now processing GY94 model-------\n");
Optimize (res0,lf0);

lnLg = res0[1][0];
npg = res0[1][1];

fprintf(stdout, "\n likelihood of gy94 is ", lnLg);
fprintf("OUTPUT_RESULT_FILE", "\n likelihood of gy94 is ", lnLg);
fprintf ("OUTPUT_RESULT_FILE", "\n number of local parmeters ---->    ", res0[1][1]);
fprintf ("OUTPUT_RESULT_FILE", "\n number of global parmeters ---->   ", res0[1][2]);

AICgy = -2*lnLg+2*npg;

fprintf (stdout, "\n AIC for gy94 value ---->   ",AICgy);
fprintf ("b0099.result", "\n AIC for gy94 value ---->   ", AICgy);

/*=================================================================================================================*/

DataSet nucleotideSequencesA = ReadDataFile ("INPUT_FILE");

fprintf (stdout,"\n ======data set is \n", ds);
fprintf ("OUTPUT_RESULT_FILE", CLEAR_FILE);

DataSetFilter filteredDataA = CreateFilter (nucleotideSequencesA,3,"","", GeneticCodeExclusions);

fprintf (stdout, "\n now filter is ok");
fprintf ("OUTPUT_RESULT_FILE",filteredDataA);

HarvestFrequencies (FEV, filteredDataA, 1, 1, 0);
fprintf (stdout, "\n frequency ready \n");

ACCEPT_ROOTED_TREES = 1;

#include "CUSTOM_MODEL_FILE";
/* to put two model in one file , have to define a model here */
...
LikelihoodFunction lfA = (filteredDataA,givenTree);
...
fprintf (stdout, "\n likelihood is ----> ", lnL);
fprintf (stdout, "\n number of local parmeters ---->	", resA[1][1]);
fprintf (stdout, "\n number of global parmeters ---->	", resA[1][2]);
fprintf ("OUTPUT_RESULT_FILE", "\n number of local parmeters ---->	", resA[1][1]);
fprintf ("OUTPUT_RESULT_FILE", "\n number of global parmeters ---->	", resA[1][2]);
